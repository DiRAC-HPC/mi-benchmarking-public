...
if len(sys.argv) == 1:
    ...
    numOfSeries = int((len(sys.argv) - 1)/2)
    if int(len(sys.argv) - 1) % 2 != 0:
        print("need to specify input files and corresponding name")
        sys.exit()
    for fileName in sys.argv[1:int(numOfSeries + 1)]:
        inputFileNames.append(fileName)
    for name in sys.argv[int(numOfSeries + 1):]:
...
# Pass string to indicate if we are scaling with number of nodes, else use 
# number of threads.
def parse_files(listStr):
    ...
    for i in range(0, numOfSeries):  # Loop over each data series
        ...
        file_list.append([])
        file_list[i] = glob.glob(inputFileNames[i] + "*")

        threadList.append([])

        # Remove path from file names
        for j in range(0, len(file_list[i])):
            p, filename = ntpath.split(file_list[i][j])
            file_list[i][j] = filename
...
def plot_results(totalTime, speedUp, parallelEff, numSeries, xList):
    ...
    for List in threadList:
        max_threads = max(max_threads,List[-1])

    # Plot speed up
    speedUpPlot.plot(threadList[0], threadList[0], linestyle="--", lw=1.5, color="0.2")
    for i in range(0, numSeries):
        speedUpPlot.plot(threadList[i], speedUp[i], linestyle[i], label=series_names[i])

    speedUpPlot.set_ylabel("${\\rm Speed\\textendash up}$", labelpad=0.0)
    speedUpPlot.set_xlabel(xList, labelpad=0.0)
    speedUpPlot.set_xlim([0.7, max_threads + 1])
    ...
    parallelEffPlot.set_xlabel(xList, labelpad=0.0)
    parallelEffPlot.set_ylim([0, 1.1])
    parallelEffPlot.set_xlim([0.9, 10 ** (np.floor(np.log10(max_threads)) + 1.)])
    ...
    for i in range(0, numOfSeries):
        ...
        totalTimePlot.loglog(
            threadList[i], totalTime[i], linestyle[i], label=series_names[i]
        )
    ...
    totalTimePlot.set_xlabel(xList, labelpad=0.0)
    totalTimePlot.set_ylabel("${\\rm Time~to~solution}~[{\\rm ms}]$", labelpad=0.0)
    totalTimePlot.set_xlim([0.9, 10 ** (np.floor(np.log10(max_threads)) + 1.)])
...
plot_results(totalTime, speedUp, parallelEff, numOfSeries, xListName)
