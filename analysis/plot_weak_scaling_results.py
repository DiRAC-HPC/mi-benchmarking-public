...
def parse_files():
    ...
    for i in range(0, numOfSeries):  # Loop over each data series
        ...
        for j in range(0, len(file_list)):
            times = np.loadtxt(file_list[j], usecols=(11,))
            updates = np.loadtxt(file_list[j], usecols=(7,))
    ...
    for i in range(0, numOfSeries):
        for j in range(0, len(file_list)):
            ...
            parallelEff[i].append(totalTime[i][0] / totalTime[i][j])
...
def plot_results(totalTime, speedUp, parallelEff, numSeries):
    ...
    fig, axarr = plt.subplots(1, 2, figsize=(10, 5), frameon=True)
    totalTimePlot = axarr[0]
    parallelEffPlot = axarr[1]

    # Plot speed up
    #speedUpPlot.plot(threadList[0], threadList[0], linestyle="--", lw=1.5, color="0.2")
    #for i in range(0, numSeries):
    #    speedUpPlot.plot(threadList[0], speedUp[i], linestyle[i], label=series_names[i])
    ...
    #speedUpPlot.set_xscale("log")
    #speedUpPlot.set_ylim([0, 1.1])
    #speedUpPlot.set_xlim([0.9, 10 ** (np.floor(np.log10(threadList[0][-1])) + 0.5)])
    ...
    for i, txt in enumerate(threadList[0]):
	    ...
	    True
            ...
            #    (threadList[0][i], speedUp[0][i] + 0.02),
            #    color=hexcols[0],
            #)
    ...
    fig.suptitle(
        "${\\rm ~Parallel~efficiency~and~time~to~solution~for}~%d~{\\rm time\\textendash steps}$"
        % numTimesteps,
        fontsize=16,
	y = 1.
    )
...
# And plot
plt.savefig("weak_scaling", dpi=200)
plt.show()
